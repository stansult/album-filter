...
async function planDeployment({ github, context, git = execFileSync }) {
  ...
  // Use successful deployments, not previous pushes: failed tests must not erase pending changes.
  let baseline;
  ...
  if (!baseline) return { needed: true, reason: 'Deploying to establish the first successful workflow baseline.' };
  try {
    ...
    // A force-pushed/deleted baseline cannot be compared safely; publish the tested snapshot.
